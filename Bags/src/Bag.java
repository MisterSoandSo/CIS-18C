public final class Bag<T extends Object & Comparable> implements BagInterface<T> {
    private T[] bag_items;
    private int num_items;  // current position
    ...
    //personal resize - if true, double array cap
    // if false, then resize was not neccessary.
    
    
    public boolean resize() 
    {
        int temp_bagsz = this.bag_items.length;
        if(this.isFull())
        {
            T[] temp_bag_items = (T[])new Object[2*temp_bagsz]; // double storage capacity
            for(int i = 0;i<temp_bagsz; i++)
            {
                temp_bag_items[i] = this.bag_items[i];
            }
            this.bag_items = temp_bag_items;
            return true;
        }
        return false;
    }
